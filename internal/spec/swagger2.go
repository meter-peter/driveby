...
func (s *swagger2Spec) Paths() map[string]*PathItem {
	...
	for path, pathItem := range s.doc.Paths {
		...
		// Swagger 2.0 has explicit fields per method.
		if pathItem.Get != nil {
...
func (s *swagger2Spec) Components() *Components {
	...
	// Definitions -> Schemas
	for name, schema := range s.doc.Definitions {
		if schema == nil {
			continue
		}
		out.Schemas[name] = normalizeV2Schema(schema)
	}
	...
	for name, scheme := range s.doc.SecurityDefinitions {
		...
		out.SecuritySchemes[name] = &SecurityScheme{
			...
			BearerFormat: "", // Not present in Swagger 2.0
			Description: scheme.Description,
		}
	}

	// If everything is empty, return nil to signal absence.
...
func (s *swagger2Spec) ValidateStructure(ctx context.Context) error {
	// openapi2.T does not expose a Validate method in kin-openapi.
	// We perform basic structural checks only.
	if s.doc == nil {
		return nil
	}
	...
	if s.doc.Info == nil || s.doc.Info.Title == "" || s.doc.Info.Version == "" {
...
func normalizeV2Parameters(params []*openapi2.Parameter) []*Parameter {
	...
	for _, p := range params {
		...
		// Body parameters carry the schema in p.Schema, others encode type directly.
		var schema *Schema
		if p.Schema != nil {
			schema = normalizeV2Schema(p.Schema)
		} else {
			// Primitive parameter – wrap minimal info into Schema.
			schema = &Schema{
				Type:   p.Type,
				Format: p.Format,
			}
...
func normalizeV2Operation(method, path string, op *openapi2.Operation) *Operation {
	...
	// In Swagger 2.0, a single body parameter represents the request body schema.
	for _, p := range op.Parameters {
		if p != nil && p.In == "body" && p.Schema != nil {
			rb := &RequestBody{
				Description: p.Description,
...
func normalizeV2Schema(sch *openapi2.Schema) *Schema {
	if sch == nil {
		return nil
	}

	// Convert sch to JSON then to openapi3.Schema-compatible struct fields where possible.
	// openapi2.Schema in kin-openapi intentionally mirrors many OpenAPI 3 fields, so we
	// can rely on familiar names like Type, Format, MinLength, MaxLength, Pattern, Enum, etc.
	...
	var tmp struct {
		...
		Properties  map[string]*openapi2.Schema `json:"properties"`
		Items       *openapi2.Schema       `json:"items"`
	}
	...
	out := &Schema{
		Type:        tmp.Type,
		Format:      tmp.Format,
		MinLength:   tmp.MinLength,
		MaxLength:   tmp.MaxLength,
		Pattern:     tmp.Pattern,
		Min:         tmp.Minimum,
		Max:         tmp.Maximum,
		Enum:        tmp.Enum,
		Required:    append([]string{}, tmp.Required...),
		Description: tmp.Description,
	}

	if len(tmp.Properties) > 0 {
		out.Properties = make(map[string]*Schema)
		for name, prop := range tmp.Properties {
			if prop == nil {
				continue
			}
			out.Properties[name] = normalizeV2Schema(prop)
		}
	}

	if tmp.Items != nil {
		out.Items = normalizeV2Schema(tmp.Items)
	}
...
func normalizeV2Response(resp *openapi2.Response) *Response {
	...
	if resp.Schema != nil {
		r.Content = map[string]*MediaType{
			"application/json": {
				Schema: normalizeV2Schema(resp.Schema),
			},
		}
	}
	return r
