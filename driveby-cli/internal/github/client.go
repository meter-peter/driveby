...
func writeValidationReportComment(sb *strings.Builder, r *types.ValidationReport, validationMode string, gateCtx *types.GateContext) {
	...
	// When no tests ran (health-check timeout), show a clear message instead of "0/0 principles"
	if r.TotalChecks == 0 && overallFailed && gateCtx != nil {
		sb.WriteString(fmt.Sprintf("## %s %s — %s\n\n",
			statusIcon, humanizeGateName(gateCtx.GateName), gateCtx.AppName))
		sb.WriteString("**Source environment not available** — the preceding gate must pass before this gate can run.\n\n")
		sb.WriteString("The health check could not reach the API in the source namespace. ")
		sb.WriteString("This typically means the previous environment's gate failed and the API was never deployed.\n\n")
