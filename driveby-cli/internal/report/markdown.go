...
func (g *Generator) writeValidationMarkdown(file *os.File, report *types.ValidationReport) error {
...
`, report.Timestamp.Format(time.RFC3339),
	...
	principlesByCategory := make(map[string][]types.PrincipleResult)
...
func (g *Generator) writePrincipleDetails(file *os.File, principleResult types.PrincipleResult) error {
	...
	case "P006": // API Contract Testing
		if details, ok := principleResult.Details.([]types.EndpointValidation); ok {
	...
	case "P007": // Performance Requirements
		if details, ok := principleResult.Details.(*types.PerformanceMetrics); ok {
			if _, err := fmt.Fprintf(file, "\n#### Performance Metrics\n\n"); err != nil {
...
func (g *Generator) writeLoadTestMarkdown(file *os.File, metrics *types.PerformanceMetrics) error {
...
func (g *Generator) writeFunctionalTestMarkdown(file *os.File, results []types.EndpointValidation) error {
...
`, len(results),
	...
	resultsByStatus := make(map[string][]types.EndpointValidation)
...
func countStatus(results []types.EndpointValidation, status string) int {
	count := 0
	for _, result := range results {
