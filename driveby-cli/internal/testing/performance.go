...
func NewPerformanceTester(config types.ValidatorConfig) (*PerformanceTester, error) {
...
func (t *PerformanceTester) TestPerformance(ctx context.Context) (*types.ValidationReport, error) {
	defer t.cleanup()
	...
	report := &types.ValidationReport{
		...
		Timestamp:   time.Now(),
		Principles: []types.PrincipleResult{
			{
				Principle: types.CorePrinciples[6], // P007: API Performance Compliance
				Passed:    true,
				Details: &types.PerformanceMetrics{
...
func (t *PerformanceTester) runPerformanceTests(targets []vegeta.Target) (*types.PerformanceTestResult, error) {
	attacker := vegeta.NewAttacker()
	targeter := vegeta.NewStaticTargeter(targets...)
	...
	return &types.PerformanceTestResult{Performance: perfMetrics}, nil
}
