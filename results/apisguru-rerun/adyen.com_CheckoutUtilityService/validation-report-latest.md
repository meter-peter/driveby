...
- **Status:** Failed
- **Message:** Documentation quality issues found: All operations have unique operationIds: POST /originKeys; All request/response bodies have examples: POST /originKeys: request body, POST /originKeys: application/json request body, POST /originKeys: 200 application/json response; All schemas have descriptions: CheckoutUtilityRequest, CheckoutUtilityResponse
...
```json
{
    ...
    "missing_docs": {
      ...
      "All request/response bodies have examples": [
        ...
        "POST /originKeys: application/json request body",
        "POST /originKeys: 200 application/json response"
      ],
      "All schemas have descriptions": [
...
- **Status:** Failed
- **Message:** Request validation issues found: All schemas specify data types: POST /originKeys: application/json schema; All string fields have length constraints: POST /originKeys.originDomains[]: application/json schema
...
```json
{
    ...
    "missing_validation": {
      "All schemas specify data types": [
        "POST /originKeys: application/json schema"
      ],
      "All string fields have length constraints": [
...
- **Status:** Failed
- **Message:** Error handling issues found: Error responses include error details schema: POST /originKeys: 403 response, POST /originKeys: 422 response, POST /originKeys: 500 response, POST /originKeys: 400 response, POST /originKeys: 401 response
...
```json
{
    ...
    "missing_errors": {
      "Error responses include error details schema": [
        "POST /originKeys: 403 response",
        "POST /originKeys: 422 response",
        "POST /originKeys: 500 response",
        "POST /originKeys: 400 response",
        "POST /originKeys: 401 response"
      ]
    }
  }
```
...
- **Status:** Failed
- **Message:** Security validation failed: Security schemes are defined: No security schemes defined in components.securitySchemes; Global security requirements are set: No top-level security requirements defined; Operation-level security is defined: Endpoints without security: POST /originKeys
...
```json
{
    ...
    "messages": {
      ...
      "Operation-level security is defined": "Endpoints without security: POST /originKeys",
