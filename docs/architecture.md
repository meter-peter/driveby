...
+-------------------+
  |   |   |   |   |
  v   v   v   v   v
P001 P002 P003 ... P009   (each in its own file)
         |
         v
+-------------------+
...
│  GitHub                                                     │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ novelcore/perfect-api                                 │   │
│  │ (source code + manifests/)                            │   │
│  └──────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────┘
         │ webhook (promotion PRs)        │ GitOps sync (env branches)
         ▼                                ▼
┌────────────────────────────────────────────────────────────┐
│  Kubernetes Cluster                                         │
│                                                             │
...
│    Deployment (autoSync)         Deployment (autoMerge: false) │
│    ┌─────────────────┐          ┌─────────────────┐        │
...
**All Argo Events, Workflow, RBAC, and Promoter resources are auto-generated by the XSDLC Crossplane composition** — no manual YAML. A single `XSDLC` CR (~35 lines) generates ~34 resources including EventBus, EventSources, Sensors, Ingresses, WorkflowTemplates, RBAC, ScmProvider, GitRepository, PromotionStrategy, ArgoCD Applications, environment branches, and the dedicated GitOps repository.

**ArgoCD** manages the application environments (`perfect-api-dev` with autoSync, `perfect-api-staging` with gate, `perfect-api-prod` with auto-sync, autoMerge: false) from the `novelcore/perfect-api-gitops` repo (environment branches).

See [deployment-guide.md](deployment-guide.md) for installation and [quality-gate-sdlc.md](quality-gate-sdlc.md) for the XSDLC architecture.
