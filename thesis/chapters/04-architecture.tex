...
Chapter~\ref{ch:methodology} defined Documentation-Driven Testing as a two-layer methodology: three axioms and eight validation principles. This chapter describes how the DriveBy framework implements those definitions. The presentation follows the data flow through the system: from specification loading, through normalization and principle evaluation, to report generation and CLI output. Each section explains the design decisions behind a specific subsystem and references the source code that realizes them.

The chapter covers the DriveBy binary itself---its internal packages, interfaces, and data structures. Deployment into Kubernetes and GitOps pipelines is deferred to Chapter~\ref{ch:workflow}.
...
The DriveBy architecture is shaped by five design goals derived from the DDT axioms:
...
These goals map directly to the DDT axioms. Version agnosticism enables Completeness (the same checks apply regardless of specification version). Determinism by construction enforces the Axiom of Determinism. Machine-readable output satisfies Observability.
...
DriveBy is implemented as a single Go module (\texttt{github.com/meter-peter/driveby/driveby-cli}). The module is organized into nine internal packages with a strict, unidirectional dependency flow:

\begin{verbatim}
types <- spec <- loader <- principles <- testing <- engine <- cli
\end{verbatim}

No package may import a package to its left. The \texttt{types} package has zero internal dependencies, ensuring that data structures can be shared across all layers without introducing cycles. Figure~\ref{fig:dependency-flow} visualizes this dependency chain.

\begin{figure}[htbp]
\centering
\input{figures/dependency-flow.tex}
\caption{DriveBy package dependency graph. Arrows indicate import direction; color denotes architectural layer.}
...
\midrule
\texttt{types}      & Value types, report model, exit codes, config     & 495 \\
\texttt{spec}       & \texttt{APISpec} interface + version-specific adapters     & 831 \\
\texttt{loader}     & Specification loading, version detection, preprocessing & 265 \\
\texttt{principles} & Principle checkers (P001--P008), registry        & 1{,}420 \\
\texttt{testing}    & Runtime functional and performance testing        & 757 \\
\texttt{engine}     & Orchestrator: load, select, evaluate, aggregate   & 117 \\
\texttt{report}     & JSON and Markdown report generation               & 670 \\
\texttt{cli}        & Cobra command tree, flag binding, config assembly & 650 \\
...
const (
    ...
    ValidationModeFlexible ValidationMode = "flexible"
)
...
Every DDT principle is implemented as a type that satisfies the \texttt{PrincipleChecker} interface:
...
All six implemented checkers follow a consistent internal pattern. Listing~\ref{lst:p001} shows an abbreviated version of P001 (OpenAPI Specification Compliance) to illustrate the pattern.
...
\midrule
\texttt{p001\_compliance.go}   & P001: OpenAPI Compliance     & 8  & 180 \\
\texttt{p002\_documentation.go} & P002: Documentation Quality  & 10 & 197 \\
\texttt{p003\_errors.go}       & P003: Error Handling          & 6  & 234 \\
\texttt{p004\_schema.go}       & P004: Schema Definitions      & 9  & 353 \\
...
The check counts correspond to those enumerated in Section~\ref{sec:eight-principles} of Chapter~\ref{ch:methodology}. P004 is the largest checker because recursive schema traversal requires handling nested objects, arrays, allOf compositions, and enum validation at every level.
...
The \texttt{PerformanceTester} in \texttt{testing/performance.go} (205~LOC) uses the vegeta library~\cite{vegeta} to execute load tests against the API. It constructs targets from all non-destructive endpoints (excluding DELETE and PATCH), runs a configurable attack (rate, duration, concurrent users), and collects latency percentiles (P50, P95, P99), throughput (requests per second), and error rates.

The results are compared against user-defined performance targets. If P95 latency exceeds the threshold or the success rate falls below the minimum, the principle is marked as failed with specific diagnostic messages. This threshold-based approach produces binary pass/fail outcomes suitable for CI/CD quality gates, satisfying the Axiom of Observability.
...
DriveBy's CLI is built with the Cobra framework~\cite{cobra} and organized as a root command with four subcommands. Table~\ref{tab:cli-commands} summarizes the command tree.
...
\midrule
\texttt{validate-only} & minimal/strict/flexible & Run static validation only (P001--P008 based on mode) \\
...
\texttt{test-only}     & test-only & Run both functional and performance tests without static validation \\
\bottomrule
\end{tabular}
\end{table}
...
DriveBy is designed to be embedded in CI/CD pipelines as a single binary invocation. Three properties make this practical:

\textbf{Zero configuration defaults.} Minimal mode with default timeouts requires only a specification path and a base URL. A pipeline step can begin with \texttt{driveby validate-only --openapi spec.json --api-url http://localhost:8080} and progressively add strictness.

\textbf{Stdout/stderr separation.} The JSON report on stdout can be piped to \texttt{jq} for field extraction, stored as a build artifact, or forwarded to a quality dashboard. Logs on stderr provide debugging context without polluting the report stream.

\textbf{GitHub integration.} The \texttt{--github-comment} flag, combined with repository and PR number flags, enables DriveBy to post validation results as PR comments. This supports both personal access tokens and GitHub App authentication (via \texttt{--github-app-id} and \texttt{--github-private-key}), making DriveBy suitable for both individual and organizational CI configurations.
...
This chapter described the architecture of the DriveBy framework across nine internal packages. The specification abstraction layer (Section~\ref{sec:spec-abstraction}) provides version-agnostic validation through the Adapter pattern, ensuring that principle checkers operate identically on OpenAPI~3.x and Swagger~2.0 specifications. The principle checker pattern (Section~\ref{sec:principle-checker}) uses the Strategy pattern to encapsulate each DDT principle as an interchangeable validator selected by the registry based on validation mode. The engine (Section~\ref{sec:engine}) orchestrates the four-step pipeline---load, select, evaluate, aggregate---that produces deterministic, structured reports.
...
Chapter~\ref{ch:workflow} describes how this binary is deployed into Kubernetes-native GitOps pipelines, using Argo Events for event-driven triggering, Argo Workflows for orchestration, and GitOps Promoter for environment promotion based on validation results.
