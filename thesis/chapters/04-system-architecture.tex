...
The final stage---the \texttt{XSDLC} custom resource described in Section~\ref{sec:xrd-design}---declares the entire pipeline as a single Kubernetes resource: from one CR (\(\sim\)25 lines of YAML) declaring the desired end state, Crossplane's reconciliation loop provisions the complete gate infrastructure.
