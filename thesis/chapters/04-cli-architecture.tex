...
\centering
\resizebox{\textwidth}{!}{%
\input{figures/system-context.tex}%
}
...
The \texttt{PerformanceTester} in \texttt{testing/performance.go} (205~LOC) uses the vegeta library~\cite{vegeta} to execute load tests against the API. It constructs targets from non-destructive endpoints, runs a configurable load profile---vegeta's terminology refers to this as an ``attack'', but the test issues only well-formed, specification-derived requests at a configurable rate, duration, and concurrency level; no penetration testing or security probing is performed---and collects latency percentiles and throughput metrics. Results are compared against user-defined thresholds, producing binary pass/fail outcomes suitable for CI/CD quality gates. The \texttt{PrincipleChecker} wrapper for P007 that integrates these results into the unified validation report is pending.
...
To make this contract machine-checkable, DriveBy publishes a JSON Schema (Draft~07) for the \texttt{ValidationReport} format at \texttt{driveby-cli/schemas/report.\allowbreak schema.\allowbreak json} in the source repository. Downstream consumers---CI gates, dashboards, agent integrations---can validate report payloads against this schema using any standard JSON Schema validator, and the schema itself is verified against every sample report under \texttt{results/local/} during the release process so that documentation and implementation cannot drift silently.
