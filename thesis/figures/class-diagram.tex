...
\node[iface] (apispec) at (0,0) {%
  ...
  \texttt{Operations() []*Operation}\\
  \texttt{Components() *Components}\\
  \texttt{ValidateStructure() error}%
};
...
\node[iface] (pcint) at (0,-7.0) {%
  ...
  \texttt{ID() string}\\
  \texttt{Axiom() Axiom}\\
  \texttt{Severity() Severity}\\
  \texttt{Modes() []ValidationMode}\\
  \texttt{Check(spec APISpec)}\\
  \quad\texttt{PrincipleResult}%
};

\draw[use] (pcint.north) -- node[right]{consumes} (apispec.south);
\draw[use] (engine.south) to[bend left=20] node[right]{invokes} (pcint.east);
...
\node[plcheck-planned] (p006) at (-1.0,-11.3){P006\\Functional\\(planned)};
\node[plcheck-planned] (p007) at ( 2.5,-11.3){P007\\Performance\\(planned)};
...
\node[plcheck-planned, scale=0.55] at (4.0,-13.2) {planned};
\node[anchor=west, font=\tiny] at (4.65,-13.2) {dashed = defined, not yet implemented};
