...
% BYOCI boundary — labels placed clearly above and below the dashed line
% with a small vertical gap so they do not collide with it.
\draw[boundary=red!70!black] (-0.5,-0.75) -- (10.5,-0.75);
\node[font=\tiny\bfseries,text=red!60!black,anchor=south east] at (10.5,-0.55) {Developer updates dry manifests on main (BYOCI)};
\node[font=\tiny\bfseries,text=teal!60!black,anchor=north east] at (10.5,-0.95) {XSDLC hydrates, promotes, gates, syncs};
...
% Label showing hydrated output — placed in the gap between the hydrator
% box and the env-next row, in the small horizontal space between the
% staging-next arrow (centred) and the prod-next arrow (rightmost).
\node[font=\tiny,text=blue!60!black,anchor=west] at (7.0,-2.45) {manifests/ + hydrator.metadata};
...
% Active branches
\node[branch=teal] (dev)     at (0,-5.8) {environment/\\dev};
\node[branch=teal] (staging) at (5,-5.8) {environment/\\staging};
\node[branch=teal] (prod)    at (10,-5.8) {environment/\\prod};
...
\node[branch=green!60!black] (argodev) at (0,-7.0) {ArgoCD sync\\(auto)};
\node[branch=green!60!black] (argostg) at (5,-7.0) {ArgoCD sync\\(auto)};
\node[branch=green!60!black] (argoprod) at (10,-7.0) {ArgoCD sync\\(auto)};
