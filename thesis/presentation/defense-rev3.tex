...
\vspace{-0.2em}
\begin{tabular}{@{}p{0.06\textwidth} p{0.22\textwidth} p{0.66\textwidth}@{}}
\toprule
& \textbf{Answer} & \textbf{Evidence} \\
\midrule
\textbf{RQ1} & \alert{\textbf{Yes}} & \scriptsize XSDLC declares the gate; the Workflow runs DriveBy on every PR
and writes a CommitStatus that auto-merges or blocks. Promotion is reproducible
and human-free at the verdict step. \\[0.3em]

\textbf{RQ2} & \alert{\textbf{Differentially}} & \scriptsize Validation rules come from the OpenAPI \emph{standard}
(not the spec); functional test cases are mechanically derived from the spec via P006;
promotion gates are declared in the XSDLC manifest --- human-authored
but structurally typed and machine-discoverable. \\[0.3em]

\textbf{RQ3} & \alert{\textbf{Natively}} & \scriptsize One manifest $\to$ $\sim$45--50 reconciled
Kubernetes objects in $<$15\,s. \textbf{11 of 12} injected drift scenarios converged
automatically under Crossplane $+$ ArgoCD. \\[0.3em]

\textbf{RQ4} & \alert{\textbf{Yes}} & \scriptsize XSDLC reifies the entire pipeline as a Kubernetes CRD ---
discoverable, introspectable, and consumed identically by operators and
autonomous agents. \\
\bottomrule
\end{tabular}
