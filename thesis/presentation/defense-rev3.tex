...
\node[ablock, minimum width=2.7cm, fill=black!4]
  (live) at (-3.6, -0.8) {Deployed service\\\scriptsize live spec $+$ behaviour};
...
\item \textbf{\textcolor{sdtgreen}{Solution: DriveBy.}} The running service is ground
  ...
  checked against the declared spec) and its \emph{live behaviour} (real responses,
  exercised by P006/P007). \alert{No contradiction with ``spec is the source of truth'':}
  the declared spec is the committed contract; the deployed service is what actually runs,
  and divergence between them is the finding.
\end{itemize}
