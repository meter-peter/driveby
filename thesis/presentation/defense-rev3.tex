...
\item \textbf{\textcolor{sdtgreen}{Solution: DriveBy.}} The CLI treats the running
  service as ground truth: it probes the \emph{live contract}, compares it against the
  declared specification, and surfaces divergence as structured, actionable findings.
\end{itemize}
...
\vspace{-0.4em}
\begin{center}
\begin{tikzpicture}[>={Stealth[length=2.5mm]}, font=\small]
...
\textbf{\textcolor{sdtblue}{Three axioms}}\\[0.3em]
\footnotesize
...
\column{0.34\textwidth}
\textbf{\textcolor{sdtblue}{Nine principles}}\\[0.3em]
\scriptsize
\begin{tabular}{@{}ll@{}}
P001 & Compliance \\
P002 & Documentation \\
...
\textbf{\textcolor{sdtblue}{Validation modes}}\\[0.3em]
\scriptsize
\begin{itemize}\setlength\itemsep{0.2em}
...
\vspace{0.2em}
\footnotesize
$\sim$35 YAML lines $\to$ $\sim$45--50 Kubernetes objects in $<$15\,s.
The \texttt{Promoter} (Argo project) opens PRs between environment branches;
each PR triggers the declared gate checks before auto-merge.
...
spec--implementation drift cannot land in production by construction.
Closing the verdict tier with agent-driven remediation is the natural continuation --- see Future Work.
\end{center}
...
\footnotesize
\begin{itemize}\setlength\itemsep{0.25em}
\item Input to agent: \emph{only} the DriveBy JSON report. No remediation prompt. No exemplar.
\item Agent autonomously edited the spec; re-validation confirmed 4/6.
\item \alert{SDT's structured output suffices as machine-actionable feedback}
  --- empirical evidence for the framework's agent-consumable semantics.
\end{itemize}
...
\textbf{RQ1} --- \emph{Can quality assurance be operationalised without dedicated human testers gating the path?}\\
\textbf{Yes.} The XSDLC composition declares the gate; the gate Workflow executes DriveBy on every PR
and writes a CommitStatus that auto-merges or blocks. \alert{Promotion decisions are reproducible
and human-free at the verdict step.}

\vspace{0.5em}
\textbf{RQ2} --- \emph{To what extent are validation rules, test cases, and promotion gates mechanically derived from the spec?}\\
\textbf{Differentially.} (i) Validation rules are derived from the \emph{OpenAPI standard}, not the spec;
(ii) functional test cases are mechanically derived from the spec (P006);
(iii) promotion gates are declared in the XSDLC manifest --- still human-authored,
but structurally typed and machine-discoverable.

\vspace{0.5em}
\textbf{RQ3} --- \emph{How effectively does SDT compose with GitOps pipelines under continuous reconciliation?}\\
\textbf{Natively.} A single manifest produces $\sim$45--50 reconciled Kubernetes objects in $<$15\,s;
\textbf{11 of 12} injected drift scenarios converged automatically under Crossplane $+$ ArgoCD.

\vspace{0.5em}
\textbf{RQ4} --- \emph{Can the delivery pipeline itself be expressed as an ontologically structured specification?}\\
\textbf{Yes.} XSDLC reifies the entire pipeline as a Kubernetes CRD --- discoverable, introspectable,
and consumed identically by operators and autonomous agents.
...
\item \textbf{Verdict tier is open-loop.} The controller decides
  \emph{promote} or \emph{block} but does not produce a corrective action on
  the plant --- no repair, no $u$ that reduces $e(t)$.

\item \textbf{Verdict-tier feedback requires external loop closure.}
  When the gate fails, the structured report is consumed by a developer
  or agent; the controller does not itself patch the spec or the
  implementation. (Drift between gates is \emph{not} a separate limitation:
  the promotion chain actively blocks any production-bound transition
  that has not first passed P006, so divergent state cannot reach
  production by construction.)

\item \textbf{P006 and P007 are not yet PrincipleChecker-wrapped.}
  Runtime checks exist in \texttt{internal/testing/} but are not exposed
  through the same uniform interface as P001--P005, P008, P009.

\item \textbf{Single delivery cluster.} The evaluation runs against one
  ArgoCD/Crossplane cluster (\texttt{private.novelcore.org}); multi-cluster
  promotion is not exercised.

\item \textbf{Single specification format.} OpenAPI 3.x + Swagger 2.0
  only; AsyncAPI, gRPC, GraphQL are out of scope.

\item \textbf{Binary scoring.} Per-principle results are pass/fail
  --- no risk-weighted or severity-graded composite metric.
\end{itemize}
...
\column{0.48\textwidth}
\textbf{\textcolor{sdtblue}{Closing the verdict tier}}\\[0.3em]
\begin{itemize}\setlength\itemsep{0.15em}
...
\column{0.48\textwidth}
\textbf{\textcolor{sdtblue}{Extending the framework}}\\[0.3em]
\begin{itemize}\setlength\itemsep{0.15em}
...
\item Scheduled prod-environment gate --- CronWorkflow trigger over the
  same XSDLC composition, covering residual drift from out-of-band
  changes (engineering extension, not research).
\item Longitudinal agent-feedback study --- the single-iteration result
  of slide 16 generalised to multi-agent, multi-pass statistics.
\end{itemize}
...
\textbf{\textcolor{sdtblue}{Own intellectual contribution}}\\[0.3em]
\begin{itemize}\setlength\itemsep{0.15em}
...
\column{0.49\textwidth}
\textbf{\textcolor{sdtblue}{Agent-assisted scope}}\\[0.3em]
\begin{itemize}\setlength\itemsep{0.15em}
