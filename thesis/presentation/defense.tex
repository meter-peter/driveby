...
\node[rectangle, rounded corners=4pt, draw=sdtviolet!70!black,
      fill=sdtviolet!15, minimum width=3cm, minimum height=1cm,
      font=\small\bfseries, align=center]
  (xsdlc) at (0,1.6)
  {XSDLC CR\\\footnotesize\textnormal{$\sim$35 lines YAML}};

\node[nbx blue] (a1) at (-5.7,0)   {ArgoCD\\App};
\node[nbx blue] (a2) at (-3.8,0)   {Workflow\\Tpl};
\node[nbx blue] (a3) at (-1.9,0)   {Event\\Source};
\node[nbx blue] (a4) at ( 0.0,0)   {Sensor};
\node[nbx blue] (a5) at ( 1.9,0)   {Ingress};
\node[nbx blue] (a6) at ( 3.8,0)   {RBAC};
\node[nbx blue] (a7) at ( 5.7,0)   {Promoter};

\node[nbx teal] (b1) at (-5.7,-1.1) {Branches};
\node[nbx teal] (b2) at (-3.8,-1.1) {Branch\\Prot};
\node[nbx teal] (b3) at (-1.9,-1.1) {Repo\\Files};
\node[nbx teal] (b4) at ( 0.0,-1.1) {Secrets};
\node[nbx teal] (b5) at ( 1.9,-1.1) {Scm\\Provider};
\node[nbx teal] (b6) at ( 3.8,-1.1) {Git\\Repo};
\node[nbx teal] (b7) at ( 5.7,-1.1) {Svc\\Account};

\node[font=\tiny, text=gray, anchor=west] at (-6.3,-2.1)
  {\ldots $\sim$45--50 Kubernetes objects, generated in $<$15 seconds, all reconciled by Crossplane.};

\draw[->,thick,sdtviolet!60!black] (xsdlc.south) -- ++(0,-0.2) -| (a1.north);
\draw[->,thick,sdtviolet!60!black] (xsdlc.south) -- (a4.north);
\draw[->,thick,sdtviolet!60!black] (xsdlc.south) -- ++(0,-0.2) -| (a7.north);
\end{tikzpicture}
\end{center}

\vspace{0.4em}
\footnotesize
\alert{The CR \emph{is} a specification.} It declares the quality-gated
delivery pipeline in a structurally typed, discoverable Kubernetes
